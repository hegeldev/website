...
If you're not familiar, Hypothesis is the most widely used property-based testing library in the world.

One of the reasons why is because it's written in the highly-popular language, Python. And while Hypothesis isn't the first property-based testing library in Python, it is the first that achieved widespread use, due to the numerous benefits it has over other property-based testing libraries.

Some of the biggest benefits include:

* A great library of high-quality generators, and flexible tools for building on them.
* The "internal shrinking" feature <!-- or is "function" the better word? -->, which means that it will basically always give you a high-quality and readable final example. It avoids many of the pitfalls of shrinking in other property-based testing libraries, such as producing invalid test cases, requiring manually writing shrinkers, and poor quality out-of-the-box shrinking.
* A test database, which means that when a test fails, if you rerun it it will automatically fail fast in the same way.

All of this stems from the underlying model of Hypothesis, which is relatively simple. Most other libraries that claim to be Hypothesis-inspired didn't adopt the core model, and as a result don't compare. (Go's [Rapid library](https://github.com/flyingmutant/rapid) is probably the most credible port we've seen). 

But the real competitive advantage of Hypothesis is that the Hypothesis developers put an unreasonable amount of work into it. We'd love it if every language had a Hypothesis-quality property-based testing library, but not as much as we'd love not to have to maintain that for every language!  

Hegel, developed by members of the same developer team, is our attempt to solve this. We implement a reusable core that can be used from any language, and a variety of language specific libraries that can act as thin frontends to that core. This makes it possible to do the mountain of work once, rather than once per language, and opens up the possibility of great property-based testing in every language.
...
All of this is, of course, an argument for using *property-based testing* --- but why should you use Hegel in particular?
...
On the other hand, if you'd like to get started on some green field property-based testing, we think Hegel is a great place to do it. It inherits a lot of power from its Hypothesis core, and we've made it as easy to use as possible.
